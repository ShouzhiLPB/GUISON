...
struct mydsp : public dsp {
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "ks_mono.dsp");
	...
	virtual void instanceConstants(int sample_rate) {
		...
		fConst2 = 1.0f / std::max<float>(1.0f, 0.3f * fConst0);
		fConst3 = std::max<float>(1.0f, 0.001f * fConst0);
		fConst4 = 1.0f / fConst3;
		fConst5 = 1.0f / std::max<float>(1.0f, 0.01f * fConst0);
		fConst6 = 3.1415927f / fConst0;
		fConst7 = 1.0f / std::tan(9424.778f / fConst0);
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow0 = std::pow(2.0f, 0.083333336f * (static_cast<float>(fHslider1) + -69.0f));
		float fSlow1 = 0.5f * std::min<float>(std::pow(0.001f, 0.0022727272f / (fSlow0 * static_cast<float>(fHslider0))), 0.999f);
		float fSlow2 = fConst1 / fSlow0;
		int iSlow3 = static_cast<int>(fSlow2);
		int iSlow4 = std::min<int>(4097, std::max<int>(0, iSlow3));
		float fSlow5 = std::floor(fSlow2);
		float fSlow6 = fSlow5 + (1.0f - fSlow2);
		float fSlow7 = static_cast<float>(fButton0);
		int iSlow8 = fSlow7 == 0.0f;
		float fSlow9 = static_cast<float>(fHslider2);
		float fSlow10 = 1.0f / std::tan(fConst6 * (47.244095f * fSlow9 + 2e+03f));
		float fSlow11 = 1.0f - fSlow10;
		float fSlow12 = 1.0f / (fSlow10 + 1.0f);
		float fSlow13 = 0.007874016f * fSlow9;
		int iSlow14 = std::min<int>(4097, std::max<int>(0, iSlow3 + 1));
		float fSlow15 = fSlow2 - fSlow5;
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			float fTemp0 = fSlow1 * (fRec0[1] + fRec0[2]);
			fVec0[IOTA0 & 8191] = fTemp0;
			fVec1[0] = fSlow7;
			iRec1[0] = iSlow8 * (iRec1[1] + 1);
			fRec2[0] = fSlow7 + fRec2[1] * static_cast<float>(fVec1[1] >= fSlow7);
			iRec4[0] = 1103515245 * iRec4[1] + 12345;
			float fTemp1 = static_cast<float>(iRec4[0]);
			fVec2[0] = fTemp1;
			fRec3[0] = fSlow12 * (4.656613e-10f * (fTemp1 + fVec2[1]) - fSlow11 * fRec3[1]);
			fRec0[0] = fSlow15 * fVec0[(IOTA0 - iSlow14) & 8191] + fSlow13 * fRec3[0] * std::max<float>(0.0f, std::min<float>(fConst4 * fRec2[0], std::max<float>(fConst5 * (fConst3 - fRec2[0]) + 1.0f, 0.0f)) * (1.0f - fConst2 * static_cast<float>(iRec1[0]))) + fSlow6 * fVec0[(IOTA0 - iSlow4) & 8191];
			output0[i0] = static_cast<FAUSTFLOAT>(fRec0[0]);
			fRec5[0] = -(fConst9 * (fConst8 * fRec5[1] - (fRec0[8] + fRec0[9])));
			output1[i0] = static_cast<FAUSTFLOAT>(fRec5[0]);
			IOTA0 = IOTA0 + 1;
			fVec1[1] = fVec1[0];
			iRec1[1] = iRec1[0];
			fRec2[1] = fRec2[0];
	...
	#define FAUST_FILE_NAME "ks_mono.dsp"
...
ks_mono::ks_mono() : AudioStream(FAUST_INPUTS, new audio_block_t*[FAUST_INPUTS])
{
#ifdef NVOICES
    int nvoices = NVOICES;
...
void ks_mono::update(void) { updateImp<FAUST_INPUTS, FAUST_OUTPUTS>(); }

void ks_mono::setParamValue(const std::string& path, float value)
{
    fUI->setParamValue(path, value);
}

float ks_mono::getParamValue(const std::string& path)
{
